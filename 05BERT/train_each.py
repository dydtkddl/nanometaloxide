...
def main(args):
    ...
    paths = [TRAIN_VAL_LOG_PATH, TRAIN_LOSS_AVG_IMAGE_PATH, VALIDATION_METRIC_IMAGE_NANOPARTICLE_PATH,
             VALIDATION_METRIC_IMAGE_MAIN_SUBJECT_PATH, VALIDATION_METRIC_IMAGE_AVG_PATH, TEST_METRIC_LOG_PATH,
             TEST_METRIC_IMAGE_NANOPARTICLE_PATH, TEST_METRIC_IMAGE_MAIN_SUBJECT_PATH, TEST_METRIC_IMAGE_AVG_PATH,
             MODEL_WEIGHT_PATH]
    for path in paths:
        print(path)
    ...
    optimizer_nanoparticle = Adam(model.bert_nanoparticle.parameters(), lr=LR)
    optimizer_main_subject = Adam(model.bert_main_subject.parameters(), lr=LR)
    scheduler_nanoparticle = ReduceLROnPlateau(optimizer_nanoparticle, mode='min', factor=0.1, patience=2, verbose=True)
    scheduler_main_subject = ReduceLROnPlateau(optimizer_main_subject, mode='min', factor=0.1, patience=2, verbose=True)
    early_stopping_nanoparticle = EarlyStopping(patience=5, verbose=True)
    early_stopping_main_subject = EarlyStopping(patience=5, verbose=True)
    
    train_losses_nanoparticle = []
    train_losses_main_subject = []
    val_losses_nanoparticle = []
    val_losses_main_subject = []
    ...
    for epoch in tqdm(range(EPOCHS)):
        model.train()
        total_loss_nanoparticle = 0
        total_loss_main_subject = 0
        for batch in tqdm(train_loader):
            if not early_stopping_nanoparticle.early_stop:
                optimizer_nanoparticle.zero_grad()
            
            if not early_stopping_main_subject.early_stop:
                optimizer_main_subject.zero_grad()
            ...
            if not early_stopping_nanoparticle.early_stop:
                ...
                loss_is_nanoparticle.backward(retain_graph=True)
                optimizer_nanoparticle.step()
                total_loss_nanoparticle += loss_is_nanoparticle.item()
            
            if not early_stopping_main_subject.early_stop:
                loss_main_subject = criterion(outputs_main_subject, batch['main_subject_labels'].to(device))
                loss_main_subject.backward()
                optimizer_main_subject.step()
                total_loss_main_subject += loss_main_subject.item()
        
        avg_train_loss_nanoparticle = total_loss_nanoparticle / len(train_loader)
        avg_train_loss_main_subject = total_loss_main_subject / len(train_loader)
        
        train_losses_nanoparticle.append(avg_train_loss_nanoparticle)
        train_losses_main_subject.append(avg_train_loss_main_subject)
        
        print(f"Epoch {epoch + 1}/{EPOCHS}, Train Loss Nanoparticle: {avg_train_loss_nanoparticle}, Train Loss Main Subject: {avg_train_loss_main_subject}")
        
        model.eval()
        total_val_loss_nanoparticle = 0
        total_val_loss_main_subject = 0
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                if not early_stopping_nanoparticle.early_stop:
                    loss_is_nanoparticle = criterion(outputs_is_nanoparticle, batch['is_nanoparticle_labels'].to(device))
                    total_val_loss_nanoparticle += loss_is_nanoparticle.item()
                
                if not early_stopping_main_subject.early_stop:
                    loss_main_subject = criterion(outputs_main_subject, batch['main_subject_labels'].to(device))
                    total_val_loss_main_subject += loss_main_subject.item()
        ...
        avg_val_loss_nanoparticle = total_val_loss_nanoparticle / len(val_loader)
        avg_val_loss_main_subject = total_val_loss_main_subject / len(val_loader)
        
        val_losses_nanoparticle.append(avg_val_loss_nanoparticle)
        val_losses_main_subject.append(avg_val_loss_main_subject)
        
        print(f"Validation Loss Nanoparticle: {avg_val_loss_nanoparticle}, Validation Loss Main Subject: {avg_val_loss_main_subject}")
        
        if not early_stopping_nanoparticle.early_stop:
            scheduler_nanoparticle.step(avg_val_loss_nanoparticle)
            early_stopping_nanoparticle(avg_val_loss_nanoparticle, model.bert_nanoparticle)
        
        if not early_stopping_main_subject.early_stop:
            scheduler_main_subject.step(avg_val_loss_main_subject)
            early_stopping_main_subject(avg_val_loss_main_subject, model.bert_main_subject)
        
        if early_stopping_nanoparticle.early_stop and early_stopping_main_subject.early_stop:
            print("Early stopping both models")
            break
        ...
        log_file.write(f"Epoch {epoch + 1}/{EPOCHS}, Train Loss Nanoparticle: {avg_train_loss_nanoparticle}, Train Loss Main Subject: {avg_train_loss_main_subject}, "
                    f"Validation Loss Nanoparticle: {avg_val_loss_nanoparticle}, Validation Loss Main Subject: {avg_val_loss_main_subject}, "
    ...
    plt.figure()
    plt.plot(range(1, len(train_losses_nanoparticle) + 1), train_losses_nanoparticle, label='Train Loss Nanoparticle')
    plt.plot(range(1, len(train_losses_main_subject) + 1), train_losses_main_subject, label='Train Loss Main Subject')
    plt.xlabel('Epochs')
    plt.ylim(0, 1)
    plt.ylabel('Loss')
    ...
    # Combine test metrics for both labels
    both_labels = []
    both_preds = []
    for nano_label, main_label, nano_pred, main_pred in zip(all_test_is_nanoparticle_labels, all_test_main_subject_labels, all_test_is_nanoparticle_preds, all_test_main_subject_preds):
        combined_label = [1 if n == 1 and m == 1 else 0 for n, m in zip(nano_label, main_label)]
        combined_pred = [1 if n == 1 and m == 1 else 0 for n, m in zip(nano_pred, main_pred)]
        both_labels.append(combined_label)
        both_preds.append(combined_pred)
    
    both_labels = np.array(both_labels)
    both_preds = np.array(both_preds)
    
    precision_combined = precision_score(both_labels, both_preds, average='macro', zero_division=0)
    recall_combined = recall_score(both_labels, both_preds, average='macro', zero_division=0)
    f1_combined = f1_score(both_labels, both_preds, average='macro', zero_division=0)
    accuracy_combined = accuracy_score(both_labels, both_preds)
    ...
    with open(os.path.join(TEST_METRIC_LOG_PATH, SAVE_FILE_NAME + ".txt"), "w") as f:
        ...
        f.write(f"Test Precision (Combined): {precision_combined}\n")
        f.write(f"Test Recall (Combined): {recall_combined}\n")
        f.write(f"Test F1 Score (Combined): {f1_combined}\n")
        f.write(f"Test Accuracy (Combined): {accuracy_combined}\n")
    ...
    print(f"Test Precision (Combined): {precision_combined}")
    print(f"Test Recall (Combined): {recall_combined}")
    print(f"Test F1 Score (Combined): {f1_combined}")
    print(f"Test Accuracy (Combined): {accuracy_combined}")
    ...
    # Plot combined test metrics
    metrics_combined = ['Precision', 'Recall', 'F1 Score', 'Accuracy']
    values_combined = [precision_combined, recall_combined, f1_combined, accuracy_combined]
    
    plt.figure()
    plt.bar(metrics_combined, values_combined)
    plt.ylim(0, 1)
    plt.title('Test Metrics (Combined)')
    plt.grid()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--BERTNAME", type=str, default=None, help="BERT model name")
    parser.add_argument("--processID", type=str, default=None, help="Process ID for the experiment")
